...
//! Key range for watcher to watch a range of keys.
...
///   If None, the range will contain only the single key specified by `key`.
///
/// The function returns an error if `key_end` is provided but is less than or equal to `key`,
/// as this would result in an empty range.
///
/// # Returns
