...
/// Dispatcher handle for sending events or commands to the dispatcher.
///
/// This handle provides methods to:
/// - Send commands to the dispatcher for execution
/// - Send key-value changes to be broadcast to watchers
/// - Add or remove watchers that monitor specific key ranges
/// - Execute custom functions on the dispatcher
#[derive(Clone, Debug)]
...
where C: TypeConfig
{
    ...
    pub fn send_change(&self, change: KVChange<C>) {
        self.send_command(Command::Change(change));
    }
